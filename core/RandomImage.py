...
class RandomNameJson:
    ...
    def __importjson(self, ranfile: str):
        ...
        self.__notallMatchCache = {
            ...
            for randomName, v in extContent.items()
            if v[ALLMATCH] is False
        }
    ...
    def addRandomNametoJson(
        self, randomName: str, imageName: str, fileExt: str, allMatch: bool = None
    ):
        """
        將圖片名稱加入json圖庫中
        請注意:
    ...
    def changeMatch(self, randomName: str, allMatch: bool):
        """
        變更圖片名稱的匹配規則
        """
        for i in FILE_EXTENSION:
    ...
    def getRandomImageName(self, randomName: str, fileExt: str = None) -> str | None:
        """
        取得隨機圖片
        """
        if fileExt is None:
            ...
            if randomName in self.__ranjson[fileExt]:
                return random.choice(self.__ranjson[fileExt][randomName][CONTENT])
            return None
    ...
    def getRelatedWordList(self, randomName: str) -> list[str]:
        """
        取得隨機名稱的相關字串列表
        """
        for i in FILE_EXTENSION:
    ...
    def matchString(self, string: str) -> list[str]:
        ...
        for randomName, relatedWordList in self.__notallMatchCache.items():
            ...
            elif any(word in string for word in relatedWordList):
                result.append(randomName)
        if result:
            self.__logger.info(
                f'{__name__}從字串"{string}"中取得[{"、".join(i for i in result)}]。'
            )
        return result
...
class RandomImage:
    ...
    def updateRandomNameImage(self, imageName: str, randomName: str):
        try:
            ...
            if fileExt is None:
                self.__logger.info(
                    f"{imageName}的副檔名不符合隨機資料庫中的副檔名或沒有副檔名。"
                )
                return False
    ...
    def getRandomImage(self, messageContent: str) -> str | None:
        ...
        imageName = self.__RandomNameJson.getRandomImageName(randomName, fileExt)
    ...
    def getAllRandomName(
        self, fileExt: str = None, loggerEnalbe: bool = True
    ) -> list[str]:
        if fileExt is None:
            if loggerEnalbe:
    ...
    def listRandom(self, fileExt: str = None, randomName: str | None = None):
        ...
        if (fileExt is None) and (randomName is None):
            ...
            for ext in FILE_EXTENSION:
                result += (
                    f"{ext}:\n" + self.__RandomNameJson.getAllRandomNameStr(ext) + "\n"
                )
            return result
        ...
        if randomName is None:
            ...
            return (
                f"{fileExt}:\n"
                + self.__RandomNameJson.getAllRandomNameStr(fileExt)
                + "\n"
            )
        ...
        result = "、".join(self.__RandomNameJson.getImageList(randomName, fileExt))
        if result:
            result += f"\n匹配規則:{"全部符合" if self.__RandomNameJson.getMatch(randomName) else "部分符合"}\n"
        return result
