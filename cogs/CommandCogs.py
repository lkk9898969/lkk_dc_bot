...
class CommandCogs(lkkCog):
    ...
    async def adminhelp(
        ...
        if self.is_admin(Interaction=interaction):
            ...
            self.logger.info(
                f"{interaction.user.name}嘗試查看指令表但被機器人嘲笑了一番。"
            )

    @discord.app_commands.command(
        name="apply_to", description="申請新增觸發圖檔，經過我(湯麵)審核後再行更新。"
    )
    @discord.app_commands.describe(
        filename="觸發名稱，例如 吃披薩.gif", url="圖檔的URL連結。"
    )
    ...
    @discord.app_commands.command(
        name="link_help", description="教您如何將您的 WOWS帳號 與 DC水表機器人 連結。"
    )
        ...
        if message.author.bot:
            if message.content.find("ISAC stopped tracking your account") != -1:
                ...
                try:
                    ...
                    await usermessage.reply(
                        f"水表機器人尚未有該WG帳號`{result}`的近期戰績資料。"
                    )
                    self.logger.info(
                        f"提醒了{usermessage.author.name}，{result}該帳號沒有近期戰績。"
                    )
                except:
                    self.logger.error("", exc_info=True)
