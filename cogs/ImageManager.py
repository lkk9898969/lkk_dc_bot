...
class ImageManager(lkkCog):
        ...
        *,
        ...
        try:
            if imgname.endswith(".jpg"):
                ...
                self.logger.debug(
                    f"新增圖檔時檢測到觸發名不正確。觸發名:{imgname}，URL:{url}"
                )
        except:
            self.logger.error("", exc_info=True)

    async def _UpdateRandom(
        self,
        imgname: str,
        randomname: str,
        *,
        ...
        try:
            if words:
                if self.RandomImageProcesser.checkRandomName(mainword):
                    self.RandomImageProcesser.updateRelatedWord(mainword, words)
    ...
    async def UpdateImageCmd(self, ctx: commands.Context, imgname: str, url: str):
    ...
    async def UpdateRandomCmd(
        self, ctx: commands.Context, imageName: str, randomName: str
    ):
        if self.is_admin(ctx):
    ...
    async def UpdateMatchCmd(self, ctx: commands.Context, randomName: str, match: str):
        if self.is_admin(ctx):
    ...
    async def UpdateWordCmd(self, ctx: commands.Context, mainword: str, *words: str):
        if self.is_admin(ctx):
    ...
    async def listimg(
        self, interaction: discord.Interaction, ext: Optional[str] = "all"
    ):
    ...
    @app_commands.describe(
        ext="圖檔副檔名(Ex: .jpg )，若不選擇預設為輸出全部",
        random_name="指定的隨機觸發名",
    )
    @app_commands.autocomplete(ext=extAutoComplete, random_name=randomNameAutoComplete)
    async def listrandom(
        self,
        ...
        if ext is not None:
            try:
                ...
                await interaction.response.send_message(
                    content=f"請輸入正確的副檔名。", ephemeral=True
                )
                self.logger.debug(
    ...
    @app_commands.command(
        name="search_word", description="列出指定隨機觸發名的關聯字。"
    )
    @app_commands.describe(random_name="隨機觸發名。")
    @app_commands.autocomplete(random_name=randomNameAutoComplete)
    async def listrelationword(
        self, interaction: discord.Interaction, random_name: str
    ):
        try:
            if self.RandomImageProcesser.checkRandomName(random_name):
                listword = self.RandomImageProcesser.getRelatedWord(random_name)
                if listword:
                    await interaction.response.send_message(
                        content=listword, ephemeral=True
                    )
                    self.logger.info(
                ...
                await interaction.response.send_message(
                    content=f"隨機觸發名{random_name}不存在於隨機圖庫中。",
                    ephemeral=True,
                )
                self.logger.info(
    ...
    async def on_message(self, message: discord.Message):
        ...
        await self.returnimage(self.RandomImageProcesser.getRandomImage, message)
